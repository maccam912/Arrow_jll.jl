...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import Arrow_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("Arrow_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "Arrow_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"Arrow", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "Arrow_jll", "src", "Arrow_jll.jl"))
        @info("Arrow_ll dev'ed out to /root/.julia/dev/Arrow_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("8ce61222-c28f-5041-a97a-c2198fb817bf"))
platforms = [Pkg.Artifacts.unpack_platform(e, "Arrow", artifacts_toml) for e in artifacts["Arrow"]]
...
if best_platform === nothing
    @debug("Unable to load Arrow; unsupported platform $(triplet(platform_key_abi()))")
else
